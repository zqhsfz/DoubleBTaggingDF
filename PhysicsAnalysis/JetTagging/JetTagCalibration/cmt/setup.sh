...
${CMTROOT}/${CMTBIN}/cmt.exe setup -sh -pack=JetTagCalibration -version=JetTagCalibration-00-02-28 -path=/afs/cern.ch/work/q/qzeng/gbb/DerivationFramework/JETM8/20.1.5.7/PhysicsAnalysis/JetTagging  -no_cleanup $* >${cmtJetTagCalibrationtempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/${CMTBIN}/cmt.exe setup -sh -pack=JetTagCalibration -version=JetTagCalibration-00-02-28 -path=/afs/cern.ch/work/q/qzeng/gbb/DerivationFramework/JETM8/20.1.5.7/PhysicsAnalysis/JetTagging  -no_cleanup $* >${cmtJetTagCalibrationtempfile}"
  cmtsetupstatus=2
